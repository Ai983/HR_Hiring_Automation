...
-- Jobs: one row per job posting (role). Portal status and counts stored per portal.
...
-- Applicants: every applicant stored with their job (role) so you can contact them later
...
-- RLS: enable row level security (adjust policies for your auth)
alter table public.jobs enable row level security;
alter table public.applicants enable row level security;

-- Allow anon/authenticated to read and write for now (tighten later with auth)
...
-- Storage bucket for resumes (create in Dashboard → Storage or via API)
-- Bucket name: resumes
-- After creating bucket, add policy so your app can upload (e.g. authenticated or service_role)
