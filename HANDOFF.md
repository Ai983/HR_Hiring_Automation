...
- **Migration:** `supabase/migrations/PENDING_hr_attendance_regularization.sql` — **applied to the hub via the SQL editor on 2026-08-26** (the `PENDING_` name is historical; it is live). Adds:
  ...
  - `hr.regularize_attendance` was HR-email-gated via `hr.is_attendance_regularizer()`. **That email gate, and the EA leave-approval gate `hr.is_leave_approver()`, were removed on 2026-08-27** (`20260827120000`, step 1). Leave approval moved to the `is_hr_admin` role; **regularization then moved to super_admin only** (`20260827130000`, step 2) — an `hr_admin` cannot correct past days.
  - `hr.regularize_attendance(subject, date, status, reason, in_at?, out_at?, prev?)` RPC — re-checks `hr.is_super_admin()` + mandatory reason, then upserts. No self-exclusion: only super_admin can regularize, and may correct any day including their own.
...
- **UI:** Sidebar → Employee Management → **Attendance Fix** (`src/components/panels/AttendanceRegularize.jsx`) — visible to the `super_admin` role only, form applies via the RPC, list shows every correction (who/when/why). Client gate is `ctx.is_super_admin` (from `hr.my_context()`); service `regularizeAttendance` / `fetchRegularizations` in `src/services/attendanceService.js`.
- **Prerequisite (confirmed 2026-08-26):** `hr.admin@hagerstone.com` is an active `public.employees` row with a linked `auth_user_id`.
- **Attendance Fix is super-admin only.** hr_admin runs day-to-day attendance & leave but cannot rewrite past days; that is deliberately reserved for super_admin.
